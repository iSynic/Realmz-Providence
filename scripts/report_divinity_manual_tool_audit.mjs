...
const AUDIT_BY_PAGE = {
  ...
  10: audit({
    ...
    gaps: ["Real imported-scenario fixture examples for battle-grid missing-monster repair would still improve regression confidence beyond the synthetic command fixture.", "Imported positive Battle Macro values are preserved and warned, but no real scenario fixture currently proves a scenario depends on that compatibility state."],
    followUp: ["Add real imported fixture examples for battle-grid missing-monster repair if a representative scenario surfaces.", "Keep Battle Macro field behavior aligned with the manual/source end-of-round execution model."]
  }),
  11: audit({
    status: "partial",
    domain: "Combat",
    ...
    followUp: ["Run remaining Monster Editor field-by-field label audits with Divinity screenshots/fixtures.", "Keep command-level regression coverage for set-aware editing, Copy Current To All Sets, Switch With, and Generate Variants current as fields are added.", "Keep exact Divinity Create Sets behavior fixture-gated unless a separate compatibility need appears."]
  }),
  12: audit({
    status: "partial",
  ...
  23: audit({
    ...
    gaps: ["Editing global Monster Mash itself is intentionally not implemented; Providence edits workspace-scoped icon-library copies instead.", "Classic Realmz documents an approximate per-scenario monster icon-set limit; Providence treats it as a compatibility warning, not a source-proven hard runtime cap.", "Incomplete imported scenario-owned icon pairs are diagnostic-only and do not create target rows.", "Image import creates Providence icon-library entries only; it does not mutate Monster Mash or Family Jewels."],
    followUp: ["Add a focused imported-scenario fixture for existing icon-set overrides beyond the current command-level parser/export checks.", "Consider a later explicit Monster Mash-compatible export file, separate from scenario export."]
  }),
  24: audit({
    status: "partial",
