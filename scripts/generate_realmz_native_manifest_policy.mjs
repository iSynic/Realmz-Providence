...
console.log(`${checkOnly ? "Realmz native manifest policy is current" : "Generated Realmz native manifest policy"} (contract v${policy.contractVersion}, 5 startup roles, ${baseline.triggerTables.length} trigger tables, ${optionalSemanticFiles.length} optional semantic files, ${projectPathSemanticFiles.length} project-path families, ${resourceSidecars.length} resource sidecars, ${runtimeBaselineFiles.length} runtime baseline files).`);
