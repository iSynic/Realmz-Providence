...
const nativeLayoutConsumerPaths = [
  browserBinaryWritersPath,
  browserParserPath,
  scenarioSeedMapCompilerPath,
  scenarioSeedScriptCompilerPath,
  rustProjectPath,
  rustMapsPath,
  rustRandomLevelsPath,
  rustActionPointsPath
];
for (const consumerPath of nativeLayoutConsumerPaths) {
  expect(fs.readFileSync(consumerPath, "utf8").includes("REALMZ_NATIVE_LAYOUT"), `${path.relative(root, consumerPath)} must consume REALMZ_NATIVE_LAYOUT`);
}
