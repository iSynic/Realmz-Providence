...
validateCompatibilityRanges(nativeLayout.rules?.raceCompatibilityRanges, nativeLayout.rules.raceRecordBytes, "nativeLayout.rules.raceCompatibilityRanges");
...
validateCompatibilityRanges(nativeLayout.rules?.casteCompatibilityRanges, nativeLayout.rules.casteRecordBytes, "nativeLayout.rules.casteCompatibilityRanges");
...
const nativeCompatibilityRanges = {
  race: nativeLayout.rules.raceCompatibilityRanges,
  caste: nativeLayout.rules.casteCompatibilityRanges
};
...
const rustCompatibilityRanges = [
  ...
  ...nativeCompatibilityRanges.race.map((range) => `    RealmzNativeCompatibilityRange { offset: ${range.offset}, bytes: ${range.bytes} },`),
  "];",
  "",
  "#[rustfmt::skip]",
  "pub const CASTE_COMPATIBILITY_RANGES: &[RealmzNativeCompatibilityRange] = &[",
  ...nativeCompatibilityRanges.caste.map((range) => `    RealmzNativeCompatibilityRange { offset: ${range.offset}, bytes: ${range.bytes} },`),
  "];"
].join("\n");
...
const typescriptWithCompatibilityRanges = typescript.replace(
  "export const AUTHORED_SCENARIO_ITEM_RECORDS",
  `export const REALMZ_NATIVE_COMPATIBILITY_RANGES = ${JSON.stringify(nativeCompatibilityRanges, null, 2)} as const;\n\nexport const AUTHORED_SCENARIO_ITEM_RECORDS`
);
expect(typescriptWithCompatibilityRanges !== typescript, "TypeScript policy template is missing its compatibility-range insertion point");

const rustWithNativeLayout = rust.replace(
  "pub const AUTHORED_SCENARIO_ITEM_RECORDS",
  `${rustNativeLayout}\n\n${rustCompatibilityRanges}\n\npub const AUTHORED_SCENARIO_ITEM_RECORDS`
);
expect(rustWithNativeLayout !== rust, "Rust policy template is missing its native-layout insertion point");

writeOrCheck(typescriptPath, typescriptWithCompatibilityRanges);
...
const nativeLayoutConsumers = [
  ...
  [rustExporterPath, ["SCENARIO_SHELL_BYTES", "LAND_LAYOUT_BYTES", "TILE_SOLIDS_BYTES", "CUSTOM_LANDLOOK_METADATA_BYTES", "MAPSTATS_RECORD_BYTES", "MAPSTATS_RECORDS", "LANDLOOK_RANGE_HEADER_BYTES", "LANDLOOK_RANGE_SLOT_BYTES", "LANDLOOK_RANGE_SLOTS", "SPELL_OVERRIDE_RECORDS", "RACE_COMPATIBILITY_RANGES", "CASTE_COMPATIBILITY_RANGES"]],
  ...
  [rustSemanticCommonPath, ["SCENARIO_SHELL_BYTES", "SCENARIO_CONTACT_INFO_BYTES", "SCENARIO_RESTRICTIONS_BYTES", "GLOBAL_MACRO_HOOK_BYTES", "MAP_RECORD_BYTES", "EXTRACODE_BYTES", "SIMPLE_ENCOUNTER_BYTES", "COMPLEX_ENCOUNTER_BYTES", "BATTLE_BYTES", "MONSTER_BYTES", "MONSTER_DESCRIPTION_BYTES", "ITEM_BYTES", "TREASURE_BYTES", "SHOP_BYTES", "MESSAGE_BYTES", "OPTION_LABEL_BYTES", "THIEF_ENCOUNTER_BYTES", "TIMED_ENCOUNTER_BYTES", "SPELL_BYTES", "RACE_BYTES", "CASTE_BYTES", "LAND_LAYOUT_BYTES", "TILE_SOLIDS_BYTES"]],
  [rustSemanticRecordsPath, ["SCENARIO_CONTACT_INFO_BYTES", "SCENARIO_RESTRICTIONS_BYTES", "GLOBAL_MACRO_HOOK_BYTES", "MAP_RECORD_BYTES", "MAP_RECORD_MARKERS", "MAP_RECORD_MARKER_BYTES", "SIMPLE_ENCOUNTER_BYTES", "COMPLEX_ENCOUNTER_BYTES", "BATTLE_BYTES", "MONSTER_BYTES", "MONSTER_DESCRIPTION_BYTES", "ITEM_BYTES", "TREASURE_BYTES", "SHOP_BYTES", "MESSAGE_BYTES", "OPTION_LABEL_BYTES", "THIEF_ENCOUNTER_BYTES", "TIMED_ENCOUNTER_BYTES", "SPELL_BYTES", "RACE_BYTES", "CASTE_BYTES", "TILE_SOLIDS_BYTES"]],
  [rustSemanticMapNamesPath, ["MAP_RECORD_BYTES"]],
  [rustSemanticTriggersPath, ["SIMPLE_ENCOUNTER_BYTES", "COMPLEX_ENCOUNTER_BYTES", "BATTLE_BYTES", "MONSTER_BYTES", "TREASURE_BYTES", "MESSAGE_BYTES", "TIMED_ENCOUNTER_BYTES"]],
...
function validateCompatibilityRanges(ranges, recordBytes, label) {
  expect(Array.isArray(ranges) && ranges.length > 0, `${label} must be a non-empty array`);
  let previousEnd = 0;
  for (const [index, range] of ranges.entries()) {
    expect(range && typeof range === "object" && !Array.isArray(range), `${label}[${index}] must be an object`);
    expect(Number.isInteger(range.offset) && range.offset >= 0, `${label}[${index}].offset must be a non-negative integer`);
    validatePositiveInteger(range.bytes, `${label}[${index}].bytes`);
    expect(range.offset >= previousEnd, `${label}[${index}] must be sorted and non-overlapping`);
    expect(range.offset + range.bytes <= recordBytes, `${label}[${index}] must fit within its native record`);
    previousEnd = range.offset + range.bytes;
  }
}
