...
function projectRows(project: Project, catalog?: LibraryCatalog | null, customAssets: ManagedAsset[] = []) {
  ...
  for (const usage of buildEdcdRowUsages(project, catalog)) {
    add({
      ...
      scope: "scenario",
      kind: usage.status === "missing" ? "Missing Data EDCD Row" : usage.status === "shared" ? "Shared Data EDCD Row" : "Data EDCD Storage Row",
      title: `Data EDCD Row ${usage.rowId}`,
      subtitle: `${usage.statusLabel} | ${usage.primaryActionLabel ?? usage.primaryShape ?? "Raw settings"}`,
      snippet: usage.summary,
      badges: ["Technical Storage", usage.statusLabel],
