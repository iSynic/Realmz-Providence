...
The 28-scenario baseline is green as of Phase 5.2. `Half Truth`, `The End Worlds`, and `Wrath of the Mind Lords` previously timed out during Classic start automation, but now pass through oracle-only textbox, flash-message, encounter-choice, and treasure acknowledgements. `Prelude to Pestilence` remains green, with one recorded bootstrap transient noted in the manifest.

`smoke:oracle:corpus:diagnose` defaults to expected-failure corpus entries when the baseline has any. `-Scenario` narrows it to one entry, and `-TraceLevel verbose` enables extra Classic runtime markers plus timeout artifacts. The diagnosis lane still uses Providence batch export for multiple scenarios, then runs Classic serially with isolated profiles.

`smoke:oracle:corpus:triage` is tighter than diagnosis and is intentionally limited to `Half Truth`, `The End Worlds`, and `Wrath of the Mind Lords`. It exports through Providence once, then runs four Classic lanes per scenario: exported/minimal profile, original/minimal profile, exported/seeded profile, and original/seeded profile. The seeded profile copies only support folders from the installed Oracle output tree into the isolated profile, such as `Character Files`, `Data Files`, `Data PC`, and `Realmz Music`. Triage writes `triage-summary.json` and `source-vs-export-diff.json`, then classifies each scenario as a Providence export mismatch, missing fixture setup, Classic automation limitation, unsupported scenario action behavior, Classic runtime bug, fixed by auto-ack, fixed by auto-choice, or not reproduced.

Phase 5.2 adds oracle-only `autoAckTextboxes`, `autoAckFlashMessages`, `autoChooseEncounters`, and `autoChooseTreasure` diagnostics for corpus start checks. They are active only inside `REALMZ_RUN_KIND=providence-oracle-gameplay` while a gameplay command is running and only when the gameplay script explicitly requests them. Classic uses the normal textbox and flash-message cleanup paths after logging `oracle gameplay modal phase=auto-ack` markers, simple encounter prompts log `oracle gameplay modal phase=auto-choice` with the selected item/result, and start-flow treasure screens log an auto-choice for `Done`. No OS click is synthesized at the host level and normal Realmz launches are unchanged. This lets the corpus oracle move through first-start scenario text boxes, deterministic setup questions, incidental treasure screens, and click-to-dismiss flash messages that otherwise wait indefinitely for user input.
