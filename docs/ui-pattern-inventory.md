...
- Suite, Rules, Scenario, Combat, Text, Assets, Library, Player Maps, and auxiliary map-mode headers now use `PanelHeader` for stable title, description, leading visual, project-context, count, and action regions while preserving their semantic heading level; canvas, palette, and inspector headers retain their task-specific geometry.
