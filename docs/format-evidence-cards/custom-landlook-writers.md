...
Providence can show and author scenario-custom landlook movement metadata and combat-map expansion while keeping built-in Realmz landlooks read-only. Metadata compilation is authoritative; custom atlas resource packaging remains a separate gate.
...
- Use generated `PICT 306..308` replacements for custom atlas art; keep arbitrary PICT editing and Divinity Save Tiles/Load Tile Map semantics out of normal UI.
