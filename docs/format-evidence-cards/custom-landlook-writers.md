...
Metadata writer gate is open for scenario custom landlooks. Providence can re-encode `Data Custom 1/2/3 BD` byte-identically and fixture tests prove owned two-byte mutations touch only their intended fields. Atlas replacement is open only for `PICT 306..308` generated by Providence's normalized 640 x 320 image-to-PICT path. Built-in landlooks remain read-only. `Custom 1/2/3` companion files remain preserve-only.
