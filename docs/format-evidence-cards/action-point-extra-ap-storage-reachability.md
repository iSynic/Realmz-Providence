...
Normal read/write recompiles every complete imported row from its decoded semantic fields. Both compilers retain the historical fixed-row allocation when an authoring command clears or removes the highest imported row, but removed rows become deterministic zero capacity rather than preserved byte identity. Only a final partial-record suffix is restored from the compatibility annex. A future true "compact/remove imported rows" command would need to be explicit and destructive; ordinary clear/reuse controls should continue preserving row positions.
