...
| Treasure | `Data TD` | opcode `10` reward path and booty flow | Writable shell exists; needs item picker confidence. |
...
5. **Treasure/Shops**: deepen existing shells after item picker confidence improves.
   - Runtime anchors now prove treasure source records, shop source/runtime cache split, item-family ID ranges, random treasure values, and runtime shop mutation behavior. See `item-treasure-shop-runtime-anchors.md`.
