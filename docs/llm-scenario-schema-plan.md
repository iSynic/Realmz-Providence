...
- Supported map operations: `fill`, `rect`, `line`, `path`, `border`, `room`, `road`, `river`, `stamp`, `terrainGroup`, `landSecret`, `hiddenWalkable`, `combatClearing`, and `dungeonPassage`.
...
Status: implemented for fill, rectangle, line, path, border, room, wide raw road/river paths, semantic cardinal road networks, rectangular stamps, named regions, stable landlook-aware named tile placement, reusable audited named stamps, reviewed Plains/Alternate Plains/Subterranean/Desert/Swamp/Snow water-mountain-forest terrain groups, land Secret Area state, landlook-specific stock hidden-walkable and combat-clearing terrain, directional dungeon passages, and generated Action Point map markers. Semantic roads compile connected orthogonal paths into the audited 132-146 endpoint, straight, bend, T-junction, and four-way grammar on supported outdoor landlooks. The generated all-tile adjacency audit supplies per-landlook usage, exact raw variants, directional neighbor weights, and authored examples as review evidence. Map operations reject out-of-bounds geometry, diagonal or collapsed semantic road segments, unknown or landlook-incompatible named content, unavailable variants, and non-serializable signed 16-bit tile values.
