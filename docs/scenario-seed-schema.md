...
Map operations are applied in array order. `border` supports inward `thickness`; `room` fills an interior, draws its walls, and replaces wall cells with side/offset doors; raw `road` and `river` draw a polyline with an explicit tile and optional width; and `stamp` places a rectangular two-dimensional tile pattern. `semanticRoad` accepts one or more orthogonal `paths` and compiles their combined topology into audited endpoint, straight, bend, T-junction, and four-way road tiles. Put every path that should join into one operation. It supports Plains, Alternate Plains, Subterranean, Desert, Swamp, and Snow; Castle and custom landlooks are rejected until their road grammars are audited. Bridges remain explicit named tiles or stamps rather than being inferred from surrounding water. `namedStamp` places a reusable audited multi-tile composition by stable name, with its `{x, y}` coordinate as the top-left anchor. Its vocabulary includes Castle furnishings and open doors plus stock special-tile buildings and landmarks. `long-table` and `tall-tree` provide 1-based variants. A stamp can declare a `region` together with a precise `northWest`, `northEast`, `southWest`, or `southEast` anchor, allowing roads and Action Points to target the actual interaction tile rather than an approximate approach cell. The parser rejects unknown names, landlook-incompatible compositions, unavailable variants, dungeon placement, incomplete region/anchor pairs, and footprints crossing the map edge. The registry resolves through the same built-in definitions used by the map painter and is implemented in [`src/editor/map/namedLandStamps.ts`](../src/editor/map/namedLandStamps.ts).

`landmass` creates one deterministic organic island from a center, horizontal and vertical radii, and optional `roughness` percentage. It keeps the map's existing ground inside the landmass and resolves the exterior through the selected landlook's reviewed water profile, including shoreline transitions. `terrainGroup` additionally accepts `blob` geometry for organic forest, mountain, and water regions; blob masks remove unsupported one-cell caps and close narrow edge notches before tile resolution, while path geometry uses rounded strokes instead of square dilation. Solid mountain and forest interiors ignore diagonal-only gaps so their transition art always faces the actual outside edge. `semanticRoute` connects named map regions through terrain-aware cardinal routing, avoids water and mountain barriers, prefers existing roads, and optionally introduces a small deterministic sequence of natural bends. An unreachable route returns a structured warning instead of drawing a fallback road through blocked terrain. `naturalScatter` adds deterministic sparse landlook-aware ground detail, rocks, plants, occasional landmarks, and coherent tree stamps inside rectangular, path, or blob geometry. Its `density` is an integer percentage from 1 to 20 and `spacing` is a 1 to 8 cell minimum separation; it only replaces reviewed open ground and automatically leaves clearance around roads, structures, and named regions. Castle maps default to solid outer-space tile 40. `castleRoom` resolves open floor against that solid void with audited north, east, south, west, corner, and door transitions without requiring prompt output to know their numeric IDs. Each Castle door can declare a `region`, which resolves to the door tile itself.

Teleport steps may use `map` plus `{x, y}`, or `at` to target a named region directly. Named destinations are preferred because Providence can resolve map level and coordinates together. Generation diagnostics warn when a teleport lands directly on another Action Point, especially when that target immediately teleports again. They also warn when a scenario start or Action Point is placed on generated water, a boat is placed outside reviewed water terrain, or hidden-walkable terrain is isolated from a reviewed mountain or forest structure.

`namedTile` places one audited semantic tile by stable name and resolves it through the map's landlook, so a prompt can request `land-to-cave-cave-south`, `grave`, `alchemy-table`, or another registry name without knowing the numeric tile ID. Optional `variant` is 1-based and selects among visually interchangeable or audited alternatives. Optional `region` binds that region directly to the tile, which is the preferred way to place cave and door Action Points. Generation warns when a grave operation would overwrite a semantic road. The parser rejects unknown names, names unavailable for the selected landlook, and unavailable variants rather than substituting a plausible tile. The complete stable vocabulary is enforced by the JSON Schema and implemented in [`src/editor/map/namedLandTiles.ts`](../src/editor/map/namedLandTiles.ts).

`terrainGroup` accepts `water`, `mountains`, or `forest` plus rectangular or path geometry and deterministically compiles it through the checked-in landlook terrain profile, so prompts do not need exact transition tile IDs. Reviewed directional rules take precedence over corpus and atlas matching: one-cell water paths use the audited stream ends, north/south and east/west straights, bends, and three-way forks, while broad regions use audited shorelines and full-water interiors. New terrain connects directionally to adjoining audited terrain already on the map: Providence distinguishes the water-facing and land-facing sides of shoreline pieces, preserves corner/notch topology, and normalizes Realmz state bands before matching. Reviewed semantic terrain profiles cover Plains, Alternate Plains, Subterranean, Desert, Swamp, and Snow; Castle uses its audited architectural `namedTile` vocabulary plus explicit tiles and stamps. Land maps also support independent `landSecret` hidden/revealed state. `hiddenWalkable` places landlook-valid concealed walkable terrain, including Plains/Swamp/Snow 169, Castle 96, and Desert 169 or 184. `combatClearing` places landlook-valid exploration structures whose Realmz 3 x 3 combat builds clear to walkable battle terrain, including Plains/Swamp/Snow 180-185, Castle 59-65, and Desert 180-183 or 185. Dungeon maps support directional `dungeonPassage` movement. Generated Action Points synchronize their land thousand-band or dungeon bitfield marker into the owning map cell without erasing Secret Area or passage state. Operation coordinates always use visual `{x, y}` map coordinates; Providence handles Realmz's different land and dungeon storage order internally. Operations that extend beyond the 90 x 90 field are rejected rather than clipped. Tile values must fit Realmz's signed 16-bit map-cell range (`-32768..32767`).
...
`scenario.restrictions` authors the optional native `Data RI` record directly from canonical semantics. It accepts a Classic-text `description`, `maxPartyCharacters` from 0 through 6, `maxPartyLevel` from 0 through 32767, and unique one-based `bannedRaces` and `bannedCastes` IDs from 1 through 30. Zero party or level limits mean no additional cap. Omitting `restrictions` preserves a selected template's restriction record or leaves it absent for a blank project; declaring it, including as an empty object, creates an authored 320-byte restriction record with unspecified fields neutral.
