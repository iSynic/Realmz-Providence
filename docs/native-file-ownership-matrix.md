...
| `Custom 1 Music` through `Custom 3 Music` | authored | Import standard MOD bytes as scenario-scoped managed `music` assets with explicit slots 1-3; the protected Custom Library also supplies the standard MOD Outdoor Music replacement. | Canonical music assets overlay same-name annex files. An exact match for the known legacy MADG Outdoor Music fingerprint is promoted to the bundled replacement MOD while the MADG source remains annex evidence; all other unsupported legacy music remains annex-only. | Emits the canonical MOD payload deterministically as the exact top-level filename for both Mac and Windows targets. | Omitted with an explicit export limitation because Remake scenario v3 has no scenario-music playlist contract. | [ISY-347](https://linear.app/isynic/issue/ISY-347/add-scenario-music-asset-import-authoring-and-export-support). |
| Legacy `Custom 4 Music` through `Custom 9 Music` and non-MOD custom music | annex-only | Omitted. | Preserve byte-for-byte without promoting it to authored MOD semantics. | Legacy-preserving export may copy it unchanged. | Omitted because no scenario-v3 meaning is defined. | New evidence or a versioned runtime contract only. |
| Legacy `Custom 1` through `Custom 9` companions without ` Music` | annex-only | Omitted. Custom landlook authoring uses canonical metadata plus managed PICT atlases, not these pass-through files. | Preserve byte-for-byte when present; do not infer music or landlook semantics. | Legacy-preserving export may copy them unchanged. | Omitted because no scenario-v3 meaning is defined. | New evidence issue only if a runtime consumer is proven. |
...
has canonical native output but remains an explicit Remake scenario-v3 limitation.
