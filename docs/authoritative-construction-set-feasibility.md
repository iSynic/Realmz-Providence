...
Branch validation through the eighteenth slice completed on 2026-07-18:
...
| 6. Can TypeScript and Rust derive from one schema? | **Yes, incrementally.** The investigation branch now generates the shared schema version and persisted top-level field inventory from JSON Schema and checks both models plus the Rust serializer against it. Nested domain DTO generation remains family-by-family work. |
...
The branch now also defines the persisted top-level contract in language-neutral JSON Schema. Its
generator supplies the schema-version constant to both runtimes and emits one ordered field
inventory; a conformance gate compares that inventory to both handwritten models and the Rust
serializer. This removes silent top-level drift while leaving nested DTO generation as bounded,
incremental work.
...
1. **Implemented:** define the persisted top-level project/origin contract and add model/serializer
   conformance checks.
2. Generate nested TypeScript and Rust DTO groups from that contract.
...
2. **Optional resource families:** the minimum main fork, item strings, and custom-spell strings
   ...
   incomplete.
3. **Nested generated DTOs:** the language-neutral schema now owns and checks the persisted
   top-level inventory, source-origin boundary, and schema-version constant. Nested TypeScript and
   Rust domain DTOs are still maintained manually and should migrate in bounded record-family
   groups.
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  runtime constants, and model/serializer conformance tests; continue with nested DTO families.
