...
| 2. Can fresh projects export with no `raw-sources`? | **Not in current code, but yes with a bounded refactor.** Desktop export hard-fails without the directory and browser export hard-fails without a snapshot. Both already know how to synthesize the required defaults. |
...
| 9. What is the smallest ownership proof? | A no-annex fresh project compiled twice byte-identically, reimported, then started in unmodified Realmz with movement, one message Action Point, save, and reload. |
...
- A fresh project whose compiler never creates or reads a raw snapshot has not been loaded and
  played through movement, an Action Point, save, and reload in unmodified Realmz.
- A structurally valid empty main resource fork is produced today, but the no-template minimum
  resource policy is not covered by a fresh-scenario gameplay oracle.
- Classic-Mac target packaging is structurally smoke-tested, not proven here by a fresh-scenario
  gameplay run on Classic Mac Realmz.
...
The feasibility verdict is therefore source-, fixture-, and package-smoke-backed. Full runtime
ownership remains the first implementation gate.
...
8. **Gameplay acceptance evidence:** no current test demonstrates the full fresh no-raw scenario in
   unmodified Realmz through save/reload.
...
6. Stage it through the existing Oracle harness into unmodified Realmz.
...
Exit: the smallest decisive proof passes in unmodified Realmz.
