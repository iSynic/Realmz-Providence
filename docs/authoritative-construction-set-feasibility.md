...
| 6. Can TypeScript and Rust derive from one schema? | **Yes, incrementally.** The investigation branch generates the shared schema version, persisted top-level inventory, source/origin/source-file DTOs, scenario identity/startup DTOs, shared provenance/confidence primitives, map identity/layout DTOs, and random-level/rectangle DTOs from JSON Schema. It checks both project models plus the Rust serializer; remaining map-record, record, rule, and asset DTOs can migrate family by family. |
