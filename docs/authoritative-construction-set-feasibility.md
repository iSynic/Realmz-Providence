...
| `panels/rules/ruleUtils.ts` and `projectCommands/scenarioRulesCommands.ts` | **Resolved:** fresh constructors omit `rawBytes`, template copies strip them, and blank detection uses semantic fields. | Imported library summaries may continue exposing raw byte counts as compatibility evidence. |
