...
Branch validation through the twenty-second slice completed on 2026-07-18:
...
| 6. Can TypeScript and Rust derive from one schema? | **Yes, incrementally.** The investigation branch generates the shared schema version, persisted top-level inventory, source/origin/source-file DTOs, scenario identity/startup DTOs, shared provenance/confidence primitives, and map identity/layout DTOs from JSON Schema. It checks both project models plus the Rust serializer; remaining map-record, random-level, record, rule, and asset DTOs can migrate family by family. |
...
and owns the complete source/origin/source-file, scenario identity/startup, and map identity/layout
DTO families plus their shared provenance/confidence primitives. A conformance gate compares the
inventory to both project models and the Rust serializer, rejects handwritten duplicates, fixes the
compatibility-only startup/layout payload inventories, and checks the evidence/render vocabularies.
This removes silent top-level, source-boundary, startup-metadata, provenance, and core-map drift
while leaving map-record, random-level, record, rule, and asset DTO generation as bounded,
incremental work.
...
3. **Implemented for map identity/layout:** generate the stable map/render/layout DTO group; move
   map-record, random-level, and other record families in later bounded groups.
...
3. **Remaining nested generated DTOs:** the language-neutral schema now owns and checks the
   persisted top-level inventory, complete source-origin/source-file, scenario startup, and map
   identity/layout DTO families, shared provenance/confidence primitives, and schema-version
   constant. Map-record, random-level, record, rule, and asset DTOs are still maintained manually
   and should migrate in bounded families.
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  ...
  provenance, and map identity/layout DTOs; continue with later record families.
