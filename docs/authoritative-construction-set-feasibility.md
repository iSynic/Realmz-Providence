...
Branch validation through the nineteenth slice completed on 2026-07-18:
...
| 6. Can TypeScript and Rust derive from one schema? | **Yes, incrementally.** The investigation branch generates the shared schema version, persisted top-level inventory, and complete source/origin/source-file DTO family from JSON Schema. It checks both project models plus the Rust serializer; remaining domain DTOs can migrate family by family. |
...
and owns the complete source/origin/source-file DTO family. A conformance gate compares the
inventory to both project models and the Rust serializer. This removes silent top-level and source
boundary drift while leaving other nested DTO generation as bounded, incremental work.
...
1. **Implemented:** define the persisted top-level project/origin contract and add model/serializer
   conformance checks.
2. **Implemented for source metadata:** generate nested TypeScript and Rust DTO groups from that
   contract; continue with scenario identity/startup metadata.
...
3. **Remaining nested generated DTOs:** the language-neutral schema now owns and checks the
   persisted top-level inventory, complete source-origin/source-file DTO family, and schema-version
   constant. Scenario, map, record, rule, and asset DTOs are still maintained manually and should
   migrate in bounded families.
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  runtime constants, model/serializer conformance tests, and generated source DTOs; continue with
  scenario identity/startup metadata and later record families.
