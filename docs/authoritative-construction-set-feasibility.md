...
Realmz rules baseline and overlay canonical records. The later compatibility-range slice removes
native `spare`/`spacer` words from the project model. Fresh records carry no `rawBytes`; imported records, aligned legacy
...
RDD` codecs compile canonical records with normalized Boolean bytes and deterministic zero for
native alignment padding byte 563. Annex-aware compiler helpers then restore only compatible
import identity: equivalent noncanonical Boolean encodings, hidden bytes in rectangle slots that
remain inactive, byte 563, and malformed file tails. Active rectangles removed in Providence
compile as complete zeroed slots, so annex preservation cannot resurrect a semantic deletion. Old
browser project JSON with `rawValues` remains load-tolerant but drops the field during normalization. Rust ignores that
...
The fifty-eighth slice removes `ShopRecord.rawBytes` from schema-v5 canonical projects. Browser
and desktop import decode every byte in each ordinary 3,002-byte `Data SD` row into one thousand
item IDs, one thousand quantities, and inflation; the writers compile only those semantic fields.
Dense foreign suffix records and malformed tails remain annex-owned and are appended after the
canonical shop prefix. Old project JSON remains load-tolerant: incomplete item and quantity arrays
are filled from the obsolete embedded row before `rawBytes` is discarded and never repersisted.
Browser normalization and shop edit commands enforce the same boundary. The package gate keeps a
...
The sixtieth slice removes `OptionLabelRecord.rawBytes` from schema-v5 canonical projects. Browser
and desktop import decode each complete 25-byte `Data OD` row into semantic Classic text, and both
writers deterministically compile the Str24 length, payload, and zero fill from that text. Unchanged
imported rows, noncanonical capacity and padding, and malformed tails remain annex-owned. Old
project JSON remains load-tolerant by discarding the obsolete embedded row while retaining its
already-decoded semantic text; browser normalization and every option-label edit path enforce the
same boundary. The Strings workbench now reports the fixed native row contract rather than a
record-local preserved-byte count. The package gate keeps noncanonical capacity and a malformed
tail only through the annex, while the ownership proof poison-tests obsolete embedded option-label
bytes in both compilers.

The sixty-first slice removes `BattleRecord.rawBytes` from schema-v5 canonical projects. Browser
and desktop import decode every complete 346-byte `Data BD` row into the 169-cell signed monster
grid, distance, before/after message IDs, and battle macro. Both writers deterministically compile
those semantics plus zero alignment padding at byte 339. Unchanged imported rows, noncanonical
alignment bytes, row capacity, and malformed tails remain annex-owned. Old project JSON remains
load-tolerant by discarding the obsolete embedded row while retaining its already-decoded semantic
fields; browser normalization and every battle edit path enforce the same boundary. The package
...
The sixty-fourth slice removes `SimpleEncounterRecord.rawBytes` from schema-v5 canonical projects.
Browser and desktop import decode each complete 426-byte `Data ED` row into canonical actions,
choice results, controls, prompt, four Classic-text responses, authored state, and provenance; both
writers rebuild every byte from those semantics, including deterministic zero alignment and Pascal
padding. Unchanged imported rows, row capacity, and malformed tails remain annex-owned. Old project
JSON remains load-tolerant by discarding obsolete embedded rows while retaining decoded semantics.
Browser normalization, simple-encounter commands, semantic mapping, and the generated contract
enforce the same boundary. The authoritative proof poison-tests obsolete embedded rows in both
compilers, and the imported-fixture gate continues to require exact no-edit byte identity.

The sixty-fifth slice removes `ComplexEncounterRecord.rawBytes` from schema-v5 canonical projects.
Browser and desktop import decode each complete 520-byte `Data ED2` row into canonical actions,
physical and word results, group flags, spell and item routes, controls, prompt, nine Classic-text
responses, authored state, and provenance; both writers rebuild every byte from those semantics,
including deterministic zero alignment and Pascal padding. Unchanged imported rows, row capacity,
and malformed tails remain annex-owned. Old project JSON remains load-tolerant by discarding
obsolete embedded rows while retaining decoded semantics and the older result aliases long enough
to normalize them. Browser normalization, complex-encounter commands, semantic mapping, and the
generated contract enforce the same boundary. The authoritative proof poison-tests obsolete
...
The sixty-seventh slice removes `ScenarioSpellOverride.rawBytes` from schema-v5 canonical
projects. Browser and desktop import decode every byte of each complete 30-byte `Data Spell` row
into canonical spell mechanics, availability flags, authored state, and provenance; both writers
rebuild the complete row from those semantics. Untouched imported rows, short-table shape,
noncanonical nonzero Boolean encodings, and malformed tails remain annex-owned. Old project JSON
remains load-tolerant by discarding obsolete embedded rows while retaining decoded semantics.
Browser normalization, rule editing, semantic mapping, and the generated TypeScript/Rust contract
enforce the same boundary, while custom spell names continue through the separate canonical
`STR# 5000..5006` resource path. The authoritative proof poison-tests obsolete embedded spell
...
Browser and desktop import decode the semantic fields of each complete 408-byte `Data Race` row
into canonical race mechanics. The bounded `spare[8]` and `spacer[31]` ranges are now excluded
from canonical records and restored only from the compatibility annex when an imported row is
edited. Untouched imported rows, row capacity, and malformed tails also remain annex-owned. Old
project JSON remains load-tolerant while discarding obsolete embedded rows and compatibility fields.
Browser normalization, rule editing, semantic mapping, and the generated TypeScript/Rust contract
...
The sixty-ninth slice removes `ScenarioCasteOverride.rawBytes` from schema-v5 canonical projects.
Browser and desktop import decode the semantic fields of each complete 576-byte `Data Caste` row
into canonical caste mechanics. The bounded `spare1[2]`, `spare2[2]`, and `spacer[63]` ranges are
now excluded from canonical records and restored only from the compatibility annex when an
imported row is edited. Untouched imported rows, row capacity, and malformed tails also remain
annex-owned. Old project JSON remains load-tolerant while discarding obsolete embedded rows and
compatibility fields. Browser normalization, rule editing, semantic mapping, and the generated TypeScript/Rust
contract enforce the same boundary. The authoritative proof poison-tests obsolete embedded caste
rows in both compilers, and the imported-fixture gate continues to require exact no-edit byte
identity.

The seventieth slice removes the remaining duplicate byte identity from the three modeled scenario
singletons: `ScenarioContactInfo.rawBytes`, `ScenarioRestrictions.rawBytes`, and
`ScenarioGlobalMacroHooks.rawBytes`. `Data CI` is completely represented by its eighteen Str255
slots, and `Data RI` by its description, party limits, and race/caste ban sets; both compilers
continue rebuilding their complete fixed-size files from those semantics. `Global` continues to
own only the five runtime-backed hook values; reserved slot descriptors remain non-writable. Its
reserved offsets 6..7 and 12..59, plus any
malformed tail, remain explicitly compatibility-annex-owned for imported scenarios and are zero in
fresh output. Browser normalization and desktop project-open migration discard all three obsolete
arrays before canonical persistence without losing decoded semantics. The generated TypeScript and
...
The seventy-second slice removes `ScenarioShell.rawBytes` and `trailingBytes` from schema-v5
canonical projects, covering both the scenario-named startup marker and the optional `Data CS`
security backup. Browser and desktop import now retain only the five runtime integers, both
20-byte code segments, creator text, authored state, and provenance. Both writers already compiled
an exact fresh 316-byte core without consulting embedded bytes. Untouched imported singletons
remain byte-exact through the compatibility annex, while authored imports recover only a malformed
suffix after offset 316; fresh output has no suffix. Browser normalization, desktop project-open
migration, scenario/security commands, Scenario JSON template compilation, semantic mapping, and
the Remake projection now share that boundary. The authoritative proof poisons both obsolete
...
or byte encoding into policy.

The seventy-eighth slice closes the remaining native-path duplication without broadening that
boundary. Two project-path families now declare the canonical collection, `sourceFile` field, and
inclusion filter for authored custom-landlook metadata and non-empty alternate monster sets. Both
compilers generate and enforce those expectations. The same contract names `Data ID.rsrc` and
`Data Spell.rsrc` as the default fresh-authored item-text and custom-spell-name sidecars and labels
them `semantic-updates` rather than assigning a presence predicate; actual emission decisions
remain in the resource writers. Imported
...
likewise moves its codec and tests into a
...
- Random-level settings and random-rectangle records are generated in both languages. Canonical
  projects carry semantic settings only. Imported legacy encodings, inactive-slot storage, native
  alignment byte 563, and malformed tails remain accessible only through the compatibility annex
  at native export.
...
| `panels/TextPanel.tsx` | **Resolved for messages and option labels:** the workbench consumes semantic text and reports each fixed native row contract. | Keep exact imported text-row bytes in the compatibility annex rather than the canonical editor model. |
| `panels/rules/ruleUtils.ts` and `projectCommands/scenarioRulesCommands.ts` | **Resolved for rules and scenario startup metadata:** canonical shell, security-backup, contact, restriction, and global-hook DTOs expose no embedded byte identity; template copies and edits retain semantics only; and blank detection uses semantic fields. | Imported library summaries may continue exposing raw byte counts as compatibility evidence. |
...
9. **Implemented for option labels:** generate the semantic Str24 DTO, add Scenario JSON authoring,
   compile deterministic complete rows without raw identity, discard obsolete embedded rows during
   project migration, and keep noncanonical legacy capacity and tails annex-only.
10. **Implemented for battles:** generate the 169-slot semantic battle DTO, compile all 346 row
    bytes with deterministic alignment padding, discard obsolete embedded rows during project
    migration, and keep unchanged imported rows and tails annex-only.
11. **Implemented for monsters and rule-override structure:** generate complete monster,
    description, spell, race, and caste DTOs, require exact fixed dimensions, and keep remaining
    race/caste compatibility fields optional and import-only.
12. **Implemented for simple encounters:** generate the semantic 426-byte encounter DTO, discard
    obsolete embedded rows during project migration and all edit paths, and keep unchanged imported
    rows, capacity, and malformed tails only in the compatibility annex.
13. **Implemented for complex encounters:** generate the semantic 520-byte encounter DTO, discard
...
14. **Implemented for thief encounters:** generate the semantic 118-byte encounter DTO, discard
    ...
    annex.
15. **Implemented for spell overrides:** generate the semantic 30-byte spell DTO without embedded
...
16. **Implemented for race overrides:** remove duplicate 408-byte row identity and the bounded
    `spare[8]` and `spacer[31]` compatibility fields from canonical records; discard obsolete raw
    rows and compatibility fields during project migration and all edit paths; deterministically
    zero those ranges for fresh output; and restore them only from the annex for imported edits.
17. **Implemented for caste overrides:** remove duplicate 576-byte row identity and the bounded
    `spare1[2]`, `spare2[2]`, and `spacer[63]` compatibility fields from canonical records; discard
    obsolete raw rows and compatibility fields during migration and edit paths; deterministically
    zero those ranges for fresh output; and restore them only from the annex for imported edits.
18. **Implemented for scenario metadata singletons:** remove duplicate `Data CI`, `Data RI`, and
...
20. **Implemented for scenario shell/security metadata:** remove duplicate 316-byte singleton and
...
26. **Implemented for project-derived and sidecar paths:** define canonical collection/path-field
...
| `Data RD` | Fully generated semantic records + legacy annex ranges/tail | Generate one random-level record per land level | Canonical records contain settings and active rectangles only. Fresh output owns semantic bytes 0..562 and 564..643, including normalized Boolean encodings, with zero at native padding byte 563. Imported export may restore equivalent noncanonical Boolean bytes, data in still-inactive rectangle slots, byte 563, and a malformed tail only from the annex; deleting an active rectangle zeroes its complete slot. |
...
4. **Preserved bytes inside records:** resolved for whole source-record and source-file identity in
   ...
   identity or bounded `spare`/`spacer` fields; those native ranges are restored only from an
   imported compatibility annex. Marker and `Data CS` identity/tails and
