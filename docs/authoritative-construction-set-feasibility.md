...
Branch validation through the twenty-fifth slice completed on 2026-07-18:

- full Rust suite: 210 passed, 2 ignored;
- full TypeScript suite: 539 passed, plus typecheck;
...
The aggregate `npm run check` currently stops after the 539 passing TypeScript tests because the
...
| 6. Can TypeScript and Rust derive from one schema? | **Yes, incrementally.** The investigation branch generates the shared schema version, persisted top-level inventory, source/origin/source-file DTOs, scenario identity/startup DTOs, shared provenance/confidence primitives, map identity/layout and map-record DTOs, random-level/rectangle DTOs, and scenario-item DTOs from JSON Schema. It checks both project models plus the Rust serializer; remaining record, rule, and asset DTOs can migrate family by family. |
...
- Map-record marker, rectangle, and metadata DTOs plus complete scenario-item records are generated
  in both languages. Fresh records omit `rawBytes`; the item writer owns all 100 `Data NI` bytes,
  including the seven source-backed spare words.
...
| `projectCommands/mapCommands.ts` and target/rules record constructors | Map-record and scenario-item dependencies are **resolved**; remaining target/rules constructors may maintain or initialize record-shaped `rawBytes`. | Continue the semantic-record/compiler-buffer pattern family by family. |
...
random-level/rectangle, map-record/marker/rectangle, and scenario-item DTO families plus their shared
...
source-boundary, startup-metadata, provenance, core-map, and item-record drift while leaving the
...
6. Keep parser, compiler, validator, and UI behavior in handwritten modules.
...
3. **Remaining nested generated DTOs:** the language-neutral schema now owns and checks the
   ...
   identity/layout, random-level/rectangle, map-record, and scenario-item DTO families, shared
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  ...
  provenance, map identity/layout, random-level/rectangle, map-record, and scenario-item DTOs;
