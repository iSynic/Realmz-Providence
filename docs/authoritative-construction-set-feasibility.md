...
Branch validation through the twenty-sixth slice completed on 2026-07-18:

- full Rust suite: 214 passed, 2 ignored;
- full TypeScript suite: 544 passed, plus typecheck;
...
The aggregate `npm run check` currently stops after the 544 passing TypeScript tests because the
module-size baseline reports unrelated pre-existing ISY-319/320/321 growth in map, assembly,
and CSS files. The random-level and scenario-item codec extractions add no new module-size
violation; moving `Data NI` out of `economy.rs` also removes that file from the current violation
list. Architecture, lint, unit, typecheck, UI audit, production build, scenario proof, package
...
| 6. Can TypeScript and Rust derive from one schema? | **Yes, incrementally.** The investigation branch generates the shared schema version, persisted top-level inventory, source/origin/source-file DTOs, scenario identity/startup DTOs, shared provenance/confidence primitives, map identity/layout and map-record DTOs, random-level/rectangle DTOs, and complete scenario-item and treasure DTOs from JSON Schema. It checks both project models plus the Rust serializer; remaining record, rule, and asset DTOs can migrate family by family. |
...
- Map-record marker, rectangle, and metadata DTOs plus complete scenario-item and treasure records
  are generated in both languages. Fresh records omit `rawBytes`; the item writer owns all 100
  `Data NI` bytes, including the seven source-backed spare words, and the treasure writer owns all
  48 `Data TD` bytes.
...
| `projectCommands/mapCommands.ts` and target/rules record constructors | Map-record, scenario-item, and treasure dependencies are **resolved**; remaining target/rules constructors may maintain or initialize record-shaped `rawBytes`. | Continue the semantic-record/compiler-buffer pattern family by family. |
...
random-level/rectangle, map-record/marker/rectangle, scenario-item, and treasure DTO families plus
...
source-boundary, startup-metadata, provenance, core-map, item-record, and treasure-record drift while leaving the
...
7. Keep parser, compiler, validator, and UI behavior in handwritten modules.
...
3. **Remaining nested generated DTOs:** the language-neutral schema now owns and checks the
   ...
   identity/layout, random-level/rectangle, map-record, scenario-item, and treasure DTO families, shared
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  ...
  provenance, map identity/layout, random-level/rectangle, map-record, scenario-item, and treasure DTOs;
