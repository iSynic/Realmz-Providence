...
Branch validation through the twenty-ninth slice completed on 2026-07-18:

- full Rust suite: 225 passed, 2 ignored;
- full TypeScript suite: 558 passed, plus typecheck;
...
The aggregate `npm run check` currently stops after the 558 passing TypeScript tests because the
module-size baseline reports unrelated pre-existing ISY-319/320/321 growth in map, assembly,
and CSS files. The random-level, scenario-item, shop, message, and option-label codec extractions
add no new module-size violation; moving `Data NI` and `Data SD` out of `economy.rs` also removes
that file from the current violation list. Architecture, lint, unit, typecheck, UI audit, production
...
- Map-record marker, rectangle, and metadata DTOs plus complete scenario-item, treasure, shop,
  message, and option-label records are generated in both languages. Fresh records omit `rawBytes`;
  the item writer owns all
  ...
  message writer owns all 256 canonical `Data SD2` bytes, and the option-label writer owns all 25
  canonical `Data OD` bytes.
...
random-level/rectangle, map-record/marker/rectangle, scenario-item, treasure, shop, message, and
option-label DTO families plus their shared provenance/confidence primitives. A
...
message-record, and option-label-record drift while leaving the other record, rule, and asset DTO
families as bounded, incremental work.
...
9. **Implemented for option labels:** generate the semantic Str24 DTO, add Scenario JSON authoring,
   ...
   annex-only.
10. Keep parser, compiler, validator, and UI behavior in handwritten modules.
...
3. **Remaining nested generated DTOs:** the language-neutral schema now owns and checks the
   ...
   identity/layout, random-level/rectangle, map-record, scenario-item, treasure, shop, message, and
   option-label DTO families, shared provenance/confidence primitives, and schema-version constant.
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  ...
  shop, message, and option-label DTOs; continue with later record families.
