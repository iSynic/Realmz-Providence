...
Branch validation through the twenty-first slice completed on 2026-07-18:
...
| 6. Can TypeScript and Rust derive from one schema? | **Yes, incrementally.** The investigation branch generates the shared schema version, persisted top-level inventory, source/origin/source-file DTO family, scenario identity/startup DTO family, and shared provenance/confidence primitives from JSON Schema. It checks both project models plus the Rust serializer; remaining record, rule, map, and asset DTOs can migrate family by family. |
...
and owns the complete source/origin/source-file and scenario identity/startup DTO families plus
their shared provenance/confidence primitives. A conformance gate compares the inventory to both
project models and the Rust serializer, rejects handwritten duplicates, fixes the compatibility-only
startup payload inventory, and checks the evidence vocabulary. This removes silent top-level,
source-boundary, startup-metadata, and provenance drift while leaving record, rule, map, and asset
DTO generation as bounded, incremental work.
...
3. **Remaining nested generated DTOs:** the language-neutral schema now owns and checks the
   ...
   families, shared provenance/confidence primitives, and schema-version constant. Map, record,
   rule, and asset DTOs are still maintained manually and should migrate in bounded families.
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  ...
  and provenance DTOs; continue with later record families.
