...
Branch validation through the twenty-eighth slice completed on 2026-07-18:

- full Rust suite: 222 passed, 2 ignored;
- full TypeScript suite: 553 passed, plus typecheck;
...
The aggregate `npm run check` currently stops after the 553 passing TypeScript tests because the
module-size baseline reports unrelated pre-existing ISY-319/320/321 growth in map, assembly,
and CSS files. The random-level, scenario-item, shop, and message codec extractions add no new
module-size violation; moving `Data NI` and `Data SD` out of `economy.rs` also removes that file from the current
...
- Map-record marker, rectangle, and metadata DTOs plus complete scenario-item, treasure, shop, and message
  ...
  message writer owns all 256 canonical `Data SD2` bytes.
...
| `panels/TextPanel.tsx` | **Resolved for messages:** authored records no longer create or consult `rawBytes`; imported byte counts remain legacy evidence. Option labels still expose per-record preservation. | Move imported message byte counts to compatibility-annex provenance when the evidence UI is next revised. |
...
| `projectCommands/mapCommands.ts` and target/rules record constructors | Map-record, scenario-item, treasure, shop, and message dependencies are **resolved**; remaining target/rules constructors may maintain or initialize record-shaped `rawBytes`. | Continue the semantic-record/compiler-buffer pattern family by family. |
...
random-level/rectangle, map-record/marker/rectangle, scenario-item, treasure, shop, and message DTO families plus
...
and message-record drift while leaving the
...
9. Keep parser, compiler, validator, and UI behavior in handwritten modules.
...
3. **Remaining nested generated DTOs:** the language-neutral schema now owns and checks the
   ...
   identity/layout, random-level/rectangle, map-record, scenario-item, treasure, shop, and message DTO families, shared
...
- **Implemented at the top level:** establish the versioned canonical project schema, generated
  ...
  provenance, map identity/layout, random-level/rectangle, map-record, scenario-item, treasure, shop, and message DTOs;
