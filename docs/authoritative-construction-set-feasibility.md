...
Branch validation through the thirty-ninth slice completed on 2026-07-19:

- full Rust suite: 242 passed, 2 ignored;
- full TypeScript suite: 596 passed, plus typecheck;
...
The aggregate `npm run check` currently stops after the 596 passing TypeScript tests because the
...
| 3. Which native files are completely generated? | The 316-byte scenario marker and `Data CS` cores, maps, trigger tables, random levels, ED3/EDCD, messages, options, monsters/descriptions, treasures, shops, thief/timed encounters, custom spell records/names, contact/restrictions, and most record cores have writers. The detailed matrix separates truly fresh-generated bytes from compatibility ranges and baseline-supplied capacities. |
| 4. Which still depend on preserved/placeholder/resource assumptions? | The 600-byte `Scenario` support file, main resource-fork default, blank `Data Solids`, required empty startup files, imported `Data Spell` tails, other record compatibility ranges, custom music, and arbitrary legacy resources. The marker and `Data CS` no longer require preserved bytes for fresh output; exact legacy security/publish behavior remains an acceptance unknown. |
...
4. **Preserved bytes inside records:** export-time file access is now annex-bounded, but several
   imported project records still embed unowned bytes. Scenario shell, monster, monster-description, spell, race,
   caste, contact, restriction, and global-hook export no longer consults those fields; marker and
   `Data CS` identity/tails are annex-owned, and `Global`
