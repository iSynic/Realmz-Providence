...
- full Rust suite: 186 passed, 2 ignored;
...
The current fresh-project path is not yet authoritative because it adapts fresh data to the
legacy preservation exporter by manufacturing a raw snapshot. That is a bounded architectural
coupling. It is not evidence that the project model or editor must be replaced.
...
| 3. Which native files are completely generated? | Maps, trigger tables, random levels, ED3/EDCD, messages, options, monsters/descriptions, treasures, shops, thief/timed encounters, contact/restrictions, and most record cores have writers. The detailed matrix separates truly fresh-generated bytes from compatibility ranges and baseline-supplied capacities. |
| 4. Which still depend on preserved/placeholder/resource assumptions? | The 600-byte `Scenario`, main resource-fork default, `Data CS`, blank `Data Solids`, fixed `Data NI` capacity, required empty startup files, `Data Spell` capacity/tail/names, `Data Race`/`Data Caste` suffixes and capacities, record compatibility ranges, custom music, and arbitrary legacy resources. |
| 5. Can legacy preservation be isolated? | **Yes.** Raw files are concentrated in import, open/hydration, validation inventory, packaging, and export overlay helpers. Move those behind an optional compatibility annex. |
| 6. Can TypeScript and Rust derive from one schema? | **Yes.** A versioned language-neutral project schema with generated DTOs is incremental. The existing `itemTexts` drift shows why it is needed. |
...
- The Classic-Mac target is structurally deterministic but has not been played through a stock
  ...
  modern port.
- Custom spell name resources, desktop item-text resources, arbitrary custom music, and several
  legacy sidecar families do not yet have complete fresh-authoring paths.
...
The two top-level project models nearly match, but they have already drifted:

- TypeScript `Project` has `itemTexts`;
- Rust `ProvidenceProject` and the Rust `ProjectFile` serializer do not;
- browser packaging can create/update `Data ID.rsrc` item strings;
- desktop persistence/export cannot preserve that project field.
...
| Native file/family | Current ownership | Fresh authoritative target | Evidence/remaining issue |
| --- | --- | --- | --- |
| `Data Spell` | Generated prefix + imported tail | Emit exactly 105 x 30 bytes for fresh custom spells | Writer currently sizes to highest authored ID and desktop export preserves an imported tail. Realmz reads 105 records in one `fread`; fresh output must be fixed-capacity. |
| `Data Spell.rsrc` / `.rsf` / AppleDouble form | Pass-through plus imported overlay | Build custom spell `STR# 5000..5006` resources from canonical names | Current writer updates names only when a suitable imported resource fork already exists. |
...
| `Data ID.rsrc` item strings | Browser-generated only | Add to canonical model and desktop compiler | TypeScript `itemTexts` and browser writer exist; Rust project persistence/export currently omit the field. |
...
4. Make compiler defaults explicit:
   ...
   - fixed `Data NI`, `Data Spell`, `Data Race`, and `Data Caste` capacities when those families
     are emitted;
...
8. Add `itemTexts` to the canonical schema and Rust persistence/export, or remove the unsupported
   claim until that is done.
...
1. **Mandatory preservation adapter:** both exporters reject an absent raw source even for authored
   projects.
2. **Compiler emission policy:** required zero-length files and fixed-capacity families are currently
   retained by baseline copy/overlay behavior rather than emitted from a manifest.
3. **Resource defaults and separate forks:** the main fork builder is reusable, but fresh spell/item
   string forks and a runtime-proven minimum main fork policy are incomplete.
...
- Fix the TypeScript/Rust `itemTexts` drift.
...
- Complete fixed-capacity `Data NI`, `Data Spell`, `Data Race`, and `Data Caste` policies.
- Generate custom spell and item string resources in both runtimes.
