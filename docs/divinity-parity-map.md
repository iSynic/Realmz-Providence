...
| Race Editor | Rules | Scenario `Data Race` overrides are parsed into editable source-backed fields for stats, age bands, caste permissions, item usability, descriptors, and conditions; selected Divinity non-name edits now fixture-confirm the 30 x 408-byte scenario file model; custom race names are imported for context and editable as Providence project labels, but are not exported with the scenario because Divinity writes them to global `Data Files:Custom Names.rsrc` | Isolate Race Base Attacks and usable-item flags in follow-up fixtures |
| Caste Editor | Rules | Scenario `Data Caste` overrides are parsed into editable source-backed matrices for stats, spellcasting, progression, item usability, conditions, and starting items; selected Divinity non-name edits now fixture-confirm the 30 x 576-byte scenario file model; custom caste names are imported for context and editable as Providence project labels, but are not exported with the scenario because Divinity writes them to global `Data Files:Custom Names.rsrc` | Finish fixture checks for class/icon/spellcasting/starting-item/victory fields |
